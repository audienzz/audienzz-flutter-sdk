...
class AudienzzSdkWrapper {
    fun initialize(context: Context, companyId: String, isAutomaticPpidEnabled: Boolean, result: Result){
        if (AudienzzPrebidMobile.isSdkInitialized) {
            result.success(InitializationStatus.SUCCESS)
        } else {
            AudienzzPrebidMobile.initializeSdk(context, companyId, isAutomaticPpidEnabled) { status ->
                when (status) {
